...
if ( ! function_exists( 'bootstrap_start_cleanup' ) ) :
function bootstrap_start_cleanup() {

	// Launching operation cleanup.
	add_action( 'init', 'bootstrap_cleanup_head' );

	// Remove WP version from RSS.
	add_filter( 'the_generator', 'bootstrap_remove_rss_version' );

	// Remove pesky injected css for recent comments widget.
	add_filter( 'wp_head', 'bootstrap_remove_wp_widget_recent_comments_style', 1 );

	// Clean up comment styles in the head.
	add_action( 'wp_head', 'bootstrap_remove_recent_comments_style', 1 );

	// Clean up gallery output in wp.
	add_filter( 'bootstrap_gallery_style', 'bootstrap_gallery_style' );

}
add_action( 'after_setup_theme','bootstrap_start_cleanup' );
endif;
/**
 * Clean up head.+
 * ----------------------------------------------------------------------------
...
if ( ! function_exists( 'bootstrap_cleanup_head' ) ) :
function bootstrap_cleanup_head() {
...
// Remove WP version from RSS.
if ( ! function_exists( 'bootstrap_remove_rss_version' ) ) :
function bootstrap_remove_rss_version() { return ''; }
endif;

// Remove injected CSS for recent comments widget.
if ( ! function_exists( 'bootstrap_remove_wp_widget_recent_comments_style' ) ) :
function bootstrap_remove_wp_widget_recent_comments_style() {
...
if ( ! function_exists( 'bootstrap_remove_recent_comments_style' ) ) :
...
if ( ! function_exists( 'bootstrap_gallery_style' ) ) :
function bootstrap_gallery_style( $css ) {
...
if ( ! class_exists( 'bootstrap_img_rebuilder' ) ) :
	class bootstrap_img_rebuilder {
	...
	$bootstrap_img_rebuilder = new bootstrap_img_rebuilder;
...
add_action('woocommerce_before_main_content', 'bootstrap_before_content', 10);
remove_action( 'woocommerce_after_main_content', 'woocommerce_output_content_wrapper_end', 10);
add_action('woocommerce_after_main_content', 'bootstrap_after_content', 10);
?>
