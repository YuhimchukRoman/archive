...
<section class="apl-charts" id="section8">
	<div class="container">
		...
		<div class='row financial_chart'>
			<!-- PIE CHART -->
			<?php
					if (have_rows('financial_pie_charts')):
						?>
					<?php
						while (have_rows('financial_pie_charts')):
							the_row();
						?>
						<div class="col-md-4">
						...
						if (have_rows('chart_data')):
							$rand = rand();
							?>
								<canvas id="myChart_<?php echo $rand?>" width="170" height="170"></canvas>
								<script type="text/javascript">
									$(document).ready(function() {
									  var navpos = $('.apl-life-counter').offset();
									   var navbot = navpos.top + 100;
									   // console.log(navpos.top);
									    $(window).on('scroll', function() {
									      if ($(window).scrollTop() > navpos.top && $(window).scrollTop() < navbot) {
									        var data = [
									   <?php while (have_rows('chart_data')): the_row();?>
									   		{
										        value: <?php the_sub_field('amount');?>,
										        color:"<?php the_sub_field('color');?>",
										        highlight: "rgba(0,0,0,0.2)",
										        label: "<?php the_sub_field('label');?>"
										    },
											<?php endwhile;?>
										]
		...
		<div class='row nonfinancial_chart hidden_chart'>
			<!-- PIE CHART -->
			<?php
					if (have_rows('nonfinancial_pie_charts')):
						?>
					<?php
						while (have_rows('nonfinancial_pie_charts')):
							the_row();
						?>
						<div class="col-md-4">
							<?php if(get_sub_field('chart_heading')) :?>
			...
			<?php if (have_rows('files')):
					...
					<?php
						...
						<?php if(get_sub_field('file_preview')) :?>
							...
							?>
							<a class="apl-downloads-img-link" href="<?php echo $file['url'];?>">
							<div class="apl-downloads-img" style="background: url(<?php the_sub_field('file_preview')?>) no-repeat top center; background-size: cover;">
							</div></a>
						<?php endif;?>
						<?php endif;?>
